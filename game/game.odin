...
draw_ui :: proc(g: ^Game) {
    ...
    rl.DrawRectangle(color_indicator_x, right_panel_y, color_indicator_size, color_indicator_size, g.cell_color)
    rl.DrawRectangleLines(color_indicator_x, right_panel_y, color_indicator_size, color_indicator_size, rl.WHITE)
    right_panel_y += element_spacing
...
draw_pause_menu :: proc(g: ^Game) {
    ...
    instructions_x := i32(SCREEN_WIDTH / 2 - rl.MeasureText(cstring(raw_data(instructions)), 20) / 2)
    rl.DrawText(cstring(raw_data(instructions)), instructions_x, SCREEN_HEIGHT / 2 + 30, 20, rl.WHITE)
}
...
draw_menu :: proc(g: ^Game) {
    ...
    play_text := "Play"
    play_text_x := i32(play_btn.x + play_btn.width / 2 - f32(rl.MeasureText(cstring(raw_data(play_text)), 20)) / 2)
    rl.DrawText(cstring(raw_data(play_text)), play_text_x, i32(play_btn.y + 15), 20, rl.WHITE)

    // Grid size
    menu_y += menu_spacing
    grid_size_text := "Grid Size:"
    rl.DrawText(cstring(raw_data(grid_size_text)), menu_center_x - rl.MeasureText(cstring(raw_data(grid_size_text)), 20) / 2, i32(menu_y), 20, rl.WHITE)
    ...
    medium_text := "40x40"
    large_text := "60x60"
    small_text_x := i32(small_btn.x + small_btn.width / 2 - f32(rl.MeasureText(cstring(raw_data(small_text)), 18)) / 2)
    medium_text_x := i32(medium_btn.x + medium_btn.width / 2 - f32(rl.MeasureText(cstring(raw_data(medium_text)), 18)) / 2)
    large_text_x := i32(large_btn.x + large_btn.width / 2 - f32(rl.MeasureText(cstring(raw_data(large_text)), 18)) / 2)
    rl.DrawText(cstring(raw_data(small_text)), small_text_x, i32(small_btn.y + 12), 18, rl.WHITE)
    rl.DrawText(cstring(raw_data(medium_text)), medium_text_x, i32(medium_btn.y + 12), 18, rl.WHITE)
    ...
    rl.DrawText(cstring(raw_data(color_text)), menu_center_x - rl.MeasureText(cstring(raw_data(color_text)), 20) / 2, i32(menu_y), 20, rl.WHITE)

    color_btn_y := menu_y + 30
    color_btn_width: f32 = 40
    color_btn_height: f32 = 40
    color_btn_spacing: f32 = 10
    total_color_btns_width := f32(len(g.color_palette)) * (color_btn_width + color_btn_spacing) - color_btn_spacing
    ...
    for i in 0..<len(g.color_palette) {
        color_btn := rl.Rectangle{
            x = f32(menu_center_x) - total_color_btns_width / 2 + f32(i) * (color_btn_width + color_btn_spacing),
            y = f32(color_btn_y),
    ...
    rl.DrawText(cstring(raw_data(controls_text)), menu_center_x - rl.MeasureText(cstring(raw_data(controls_text)), 16) / 2, i32(SCREEN_HEIGHT - 40), 16, rl.GRAY)
}
